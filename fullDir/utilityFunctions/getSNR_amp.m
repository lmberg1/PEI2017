function SNR = getSNR_amp(cmtcode, f1, f2)
...
% snr of x component (using love waves)
if ~isempty(xNoiseData) && ~isempty(xLovData)
    SNR{1} = max(abs(xLovData))/max(abs(xNoiseData));
else
    SNR{1} = nan;
end

% snr of y component (using love waves)
if ~isempty(yNoiseData) && ~isempty(yLovData)
    SNR{2} = max(abs(yLovData)) / max(abs(yNoiseData));
else
    SNR{2} = nan;
end

% snr of x component (using rayleigh waves)
if ~isempty(zNoiseData) && ~isempty(zRayData)
    SNR{3} = max(abs(zRayData))/max(abs(zNoiseData));
else
    SNR{3} = nan;
end
