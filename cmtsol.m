function varargout = cmtsol(cmtcode)
...
%      Magnitude      earthquake magnitude (MS)
%      Location       geographical location of event (24 characters max)
...
fname = '~/Downloads/CMTCatalogueData/allData.ndk';
...
if isempty(strmatch('demo',cmtcode))
  if ~strcmp(cmtcode,'all')
    tobesaved=fullfile('~/internship/matlab',...
		       'CMTFILES',sprintf('%s.mat',cmtcode));
  else
  ...
  if exist(tobesaved,'file') 
    ...
    if strcmp(cmtcode, 'all')
      ...
      fprintf('Making MAT file for event %s\n\n', cmtcode);
    end
    ...
    for in = jj
      ...
      clear(cmtcode)
      if ~mod(in, 1000)
	fprintf('Finished making MAT files for %5.5d / %d events\n',...
		     in, length(C{4}));
      end
    end
    fprintf('Finished making MAT files for %5.5d / %d events\n',...
...
S=struct('DateTime',C{1}(in, :),...
	 ...
	 'Lon',C{9}(in),...
	 'Dep',C{10}(in),...
     'Distance', grcdist([C{9}(in) C{8}(in)], [stalon stalat]), ...
	 'Exp',C{11}(in),...
	 'M',...
